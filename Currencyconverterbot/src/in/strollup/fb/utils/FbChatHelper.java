...
import in.strollup.fb.servlet.CurrencyConvert;
...
public class FbChatHelper {
	...
	public List<String> getReplies(String senderId, String text) {
		...
		if(text.contains(",") && text.length() < 12) {
			String msg = "Converting Currency from "+text.substring(0, text.indexOf(","))+" to "+text.substring(text.indexOf(",")+1);
			Message fbMsg = getMsg(msg);
			String fbReply = getJsonReply(senderId, fbMsg);
			replies.add(fbReply);
			...
			double result = 0;
			try {
				System.out.println("calling getCurrency from bot");
				result = cc.getCurrency(text.substring(0, text.indexOf(",")), text.substring(text.indexOf(",")+1));
				System.out.println("result in bot : "+result);
			} catch (IOException e) {
				result = -1;
				e.printStackTrace();
			}
			String reply = "Result : ";
			if(result == -1)
				reply = reply.concat("Invalid Currency Code");
			else
				reply = reply.concat(Double.toString(result));
			Message rfbMsg = getMsg(reply);
			String rfbReply = getJsonReply(senderId, rfbMsg);
			replies.add(rfbReply);
		}
			...
			String welcome = "Welcome to Corrency Conversion Bot, please enter from Currentcy, to Currency ex:INR,USD";
			Message wfbMsg = getMsg(welcome);
			String wfbReply = getJsonReply(senderId, wfbMsg);
			replies.add(wfbReply);
		}
